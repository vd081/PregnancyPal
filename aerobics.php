          ...
          <div class="wrap2">
            <div class="container">
              <div class="title">
                <!---- MAIN SECTION--->
                <center>
                  <h1>Aerobics Reccomendations for you
                  </h1> 
                </center>
              </div>
              </body>
...
$dbQuery = $conn->prepare("select APoints, AerobicsReward from ExercisePoints where UserID=:UserID");
$dbParams = array(':UserID'=>$UserID);
 $dbQuery->execute($dbParams);
$dbRow = $dbQuery->fetch(PDO::FETCH_ASSOC);
    $apoints          = $dbRow['APoints']; //define points 
 $areward         = $dbRow['AerobicsReward']; // define AerobicsReward
$pointsDeduction = -20; //punishment 

if ($daysLeft == $End) { // no days left on timer
    $dbQuery = $conn->prepare("UPDATE ExercisePoints SET APoints= :points + :pointsDeduction WHERE UserID=:UserID");
  $dbParams = array(':points'=>$apoints);
...
if (isset($_POST['submit']) && ($daysLeft == 7)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET APoints= $apoints + $PointsD1 WHERE UserID=:UserID");
      $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 6)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET APoints= $apoints + $PointsD2 WHERE UserID=:UserID");
     $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 5)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET APoints= $apoints + $PointsD3 WHERE UserID=:UserID");
     $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 4)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET APoints= $apoints + $PointsD4 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 3)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET APoints= $apoints + $PointsD5 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
    $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 2)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET APoints= $apoints + $PointsD6 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
    $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 1)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET APoints= $apoints + $PointsD7 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
$dbParams = array(':UserID'=>$UserID);
 $dbQuery->execute($dbParams);
$dbRow = $dbQuery->fetch(PDO::FETCH_ASSOC);
    $apoints          = $dbRow['APoints'];
	$areward             = $dbRow['AerobicsReward'];
	
	//if user reaches 80 points - reward them
	if ( $apoints > 0 && ($apoints >= 80) ){
	echo '<h4 class="reward-text">Congratulations! You have reached 80 points! You will now be rewarded with an bonus exercise!!</h4>';
$dbQuery = $conn->prepare("UPDATE ExercisePoints SET APoints = '0', AerobicsReward = AerobicsReward + 1 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
}
...
echo"'s Points: ".$apoints." </h2>"; // Display user total points
...
//Get all Yoga recipes
        $dbQuery=$conn->prepare("select * from Exercise where ExerciseCategory='Aerobic'"); 
        $dbQuery->execute();

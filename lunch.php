...
$dbQuery = $conn->prepare("select LPoints, LunchReward from FoodPoints where UserID=:UserID");
$dbParams = array(':UserID'=>$UserID);
 $dbQuery->execute($dbParams);
$dbRow = $dbQuery->fetch(PDO::FETCH_ASSOC);
    $lpoints          = $dbRow['LPoints']; //define points 
 $lreward         = $dbRow['LunchReward']; // define reward
$pointsDeduction = -20; //punishment 

if ($daysLeft == $End) { // no days left on timer
    $dbQuery = $conn->prepare("UPDATE FoodPoints SET LPoints= :points + :pointsDeduction WHERE UserID=:UserID");
  $dbParams = array(':points'=>$lpoints);
...
if (isset($_POST['submit']) && ($daysLeft == 7)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET LPoints= $lpoints + $PointsD1 WHERE UserID=:UserID");
      $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 6)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET LPoints= $lpoints + $PointsD2 WHERE UserID=:UserID");
     $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 5)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET LPoints= $lpoints + $PointsD3 WHERE UserID=:UserID");
     $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 4)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET LPoints= $lpoints + $PointsD4 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 3)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET LPoints= $lpoints + $PointsD5 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
    $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 2)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET LPoints= $lpoints + $PointsD6 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
    $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 1)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET LPoints= $lpoints + $PointsD7 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
$dbParams = array(':UserID'=>$UserID);
 $dbQuery->execute($dbParams);
$dbRow = $dbQuery->fetch(PDO::FETCH_ASSOC);
    $lpoints          = $dbRow['LPoints'];
	$lreward         = $dbRow['LunchReward'];
	
	if ( $lpoints > 0 && ($lpoints >= 80) ){
	echo '<h4 class="reward-text">Congratulations! You have reached 80 Points! You will now be rewarded with an bonus recipe!!</h4>';
$dbQuery = $conn->prepare("UPDATE FoodPoints SET LPoints = '0', LunchReward = LunchReward + 1 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
}
...
echo"'s Points: ".$lpoints." </h2>"; // Display user total points
...
while($dbRow=$dbQuery->fetch(PDO::FETCH_ASSOC)) {
	...
	if ( ! in_array( $dbRow["RecipeID"], $user_already_select ) ){
         ...
         '</div></h4><br><img src="Lunch/'.$dbRow['RecipePicture'].'" width="150" height="150" />

          ...
          <div class="wrap2">
            <div class="container">
              <div class="title">
                <!---- MAIN SECTION--->
                <center>
                  <h1>Lunch Reccomendations for you
                  </h1> 
                </center>
              </div>
              </body>
...
$dbQuery = $conn->prepare("select DPoints, DinnerReward from FoodPoints where UserID=:UserID");
$dbParams = array(':UserID'=>$UserID);
 $dbQuery->execute($dbParams);
$dbRow = $dbQuery->fetch(PDO::FETCH_ASSOC);
    $dpoints          = $dbRow['DPoints']; //define points 
 $dreward         = $dbRow['DinnerReward']; // define reward
$pointsDeduction = -20; //punishment 

if ($daysLeft == $End) { // no days left on timer
    $dbQuery = $conn->prepare("UPDATE FoodPoints SET DPoints= :points + :pointsDeduction WHERE UserID=:UserID");
  $dbParams = array(':points'=>$dpoints);
...
if (isset($_POST['submit']) && ($daysLeft == 7)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET DPoints= $dpoints + $PointsD1 WHERE UserID=:UserID");
      $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 6)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET DPoints= $dpoints + $PointsD2 WHERE UserID=:UserID");
     $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 5)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET DPoints= $dpoints + $PointsD3 WHERE UserID=:UserID");
     $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 4)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET DPoints= $dpoints + $PointsD4 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 3)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET DPoints= $dpoints + $PointsD5 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
    $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 2)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET DPoints= $dpoints + $PointsD6 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
    $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 1)){
	$dbQuery = $conn->prepare("UPDATE FoodPoints SET DPoints= $dpoints + $PointsD7 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
  ...
  //define all keywords for Dinner
  $K1 = ("pasta");
  $K2 = ("rice");
  $K3 = ("chicken");
  $K4 = ("fish");
...
$dbParams = array(':UserID'=>$UserID);
 $dbQuery->execute($dbParams);
$dbRow = $dbQuery->fetch(PDO::FETCH_ASSOC);
    $dpoints          = $dbRow['DPoints'];
	$dreward         = $dbRow['DinnerReward'];
	
	if ( $dpoints > 0 && ($dpoints >= 80) ){
	echo '<h4 class="reward-text">Congratulations! You have reached 80 points! You will now be rewarded with an bonus recipe!!</h4>';
$dbQuery = $conn->prepare("UPDATE FoodPoints SET DPoints = '0', DinnerReward = DinnerReward + 1 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
}
...
echo"'s Points: ".$dpoints." </h2>"; // Display user total points
...
while ($dbRow=$dbQuery->fetch(PDO::FETCH_ASSOC)) //loop through all keywords 1-9
{
    for($number = 1; $number<=9; ++$number) {
        $Dinner = "D$number";
        $$Dinner = $dbRow[$Dinner]; //define row
    }
}
$cnt=0;
$max=3;
...
for($number = 1; $number<=9; ++$number) {
    ...
    $Dinner = "D$number"; //define lunch ID
    if ($$Dinner == '1')  //whether the user selected the keyword. (1=True)
    {
        $dbQuery=$conn->prepare("select * from Recipes where RecipeCategory='Dinner' AND Keyword = '".($$Keyword)."'"); //get all recipes where keyword = word user selected
        $dbQuery->execute();
...
while($dbRow=$dbQuery->fetch(PDO::FETCH_ASSOC)) {
	...
	if ( ! in_array( $dbRow["RecipeID"], $user_already_select ) ){
         ...
         '</div></h4><br><img src="Dinner/'.$dbRow['RecipePicture'].'" width="150" height="150" />

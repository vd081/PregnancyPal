          ...
          <div class="wrap2">
            <div class="container">
              <div class="title">
                <!---- MAIN SECTION--->
                <center>
                  <h1>Pilates Reccomendations for you
                  </h1> 
                </center>
              </div>
              </body>
...
$dbQuery = $conn->prepare("select PPoints, PilatesReward from ExercisePoints where UserID=:UserID");
$dbParams = array(':UserID'=>$UserID);
 $dbQuery->execute($dbParams);
$dbRow = $dbQuery->fetch(PDO::FETCH_ASSOC);
    $ppoints          = $dbRow['PPoints']; //define points 
 $yreward         = $dbRow['PilatesReward']; // define PilatesReward
$pointsDeduction = -20; //punishment 

if ($daysLeft == $End) { // no days left on timer
    $dbQuery = $conn->prepare("UPDATE ExercisePoints SET PPoints= :points + :pointsDeduction WHERE UserID=:UserID");
  $dbParams = array(':points'=>$ppoints);
...
if (isset($_POST['submit']) && ($daysLeft == 7)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET PPoints= $ppoints + $PointsD1 WHERE UserID=:UserID");
      $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 6)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET PPoints= $ppoints + $PointsD2 WHERE UserID=:UserID");
     $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 5)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET PPoints= $ppoints + $PointsD3 WHERE UserID=:UserID");
     $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 4)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET PPoints= $ppoints + $PointsD4 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 3)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET PPoints= $ppoints + $PointsD5 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
    $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 2)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET PPoints= $ppoints + $PointsD6 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
    $dbQuery->execute($dbParams);
...
if (isset($_POST['submit']) && ($daysLeft == 1)){
	$dbQuery = $conn->prepare("UPDATE ExercisePoints SET PPoints= $ppoints + $PointsD7 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
...
$dbParams = array(':UserID'=>$UserID);
 $dbQuery->execute($dbParams);
$dbRow = $dbQuery->fetch(PDO::FETCH_ASSOC);
    $ppoints          = $dbRow['PPoints'];
	$yreward             = $dbRow['PilatesReward'];
	
	//if user reaches 80 points - reward them
	if ( $ppoints > 0 && ($ppoints >= 80) ){
	echo '<h4 class="reward-text">Congratulations! You have reached 80 points! You will now be rewarded with an bonus exercise!!</h4>';
$dbQuery = $conn->prepare("UPDATE ExercisePoints SET PPoints = '0', PilatesReward = PilatesReward + 1 WHERE UserID=:UserID");
    $dbParams = array(':UserID'=>$UserID);
   $dbQuery->execute($dbParams);
}
...
echo"'s Points: ".$ppoints." </h2>"; // Display user total points
...
//Get all Yoga recipes
        $dbQuery=$conn->prepare("select * from Exercise where ExerciseCategory='Pilates'"); 
        $dbQuery->execute();
